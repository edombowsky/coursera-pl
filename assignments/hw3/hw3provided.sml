 ...
 * library function rev for reversing lists, and two library functions
...
* (notice the 2 arguments are curried). The fist argument should be applied to
* elements of the second argument in order until the first time it returns
...
* ('a -> 'b list option) -> 'a list -> 'b list option (notice the 2 arguments
* are curried). The first argument should be applied to elements of the second
* argument. If it returns NONE for any element, then the result for all_answers
* is NONE. Else thecalls to the first argument will have produced SOME lst1,
 ...
 * list matches or SOME lst where lst is the list of bindings for the frst
...
(*
 * Write a function typecheck_patterns that \type-checks" a pattern list.
 ...
 *
 * typecheck_patterns should have
 * type ((string * string * typ) list) * (pattern list) -> typ option. The frst
 * argument contains elements that look like ("foo","bar",IntT), which means
 * constructor foo makes a value of type Datatype "bar" given a value of type
 * IntT. Assume list elements all have different first fields (the constructor
 * name), but there are probably elements with the same second field (the
 * datatype name). Under the assumptions this list provides, you "type-check"
